...
init([]) ->
    ...
    %{ok, LogDir} = application:get_env(mnsr, webmachine_logs),
    % let's force the directory to exist if it doesn't already... BOOM!
    {ok, CWD} = file:get_cwd(),
    lager:warning("We're in: ~p ~n~n", [CWD]),
    WebConfig = [
                 {ip, Ip},
                 {port, Port},
                 {log_dir, "priv/log"},
                 {dispatch, Dispatch}],
    lager:warning("What's the Webmachine config: ~p~n~n", [WebConfig]),
